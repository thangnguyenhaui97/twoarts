<?php
	...
	class productController extends baseController{
		//ke thua class productModel de goi cac ham trong class do tu productController (la cac ham thao tac csdl)
		use productModel;
		...
		public function listproduct(){
			...
			$this->setView("Views/Backend/product.php", array("list_record"=>$list_record,"num_page"=>$num_page));
		...
		public function edit(){
			...
			$record = $this->fetchOne(); //ham trong class productModel
			//tao bien $form_action de dieu phoi action cua form
			$form_action = "index.php?area=admin&controller=product&action=doEdit&id=$id";
			//set file view
			$this->setView("Views/Backend/add_edit_product.php",array("record"=>$record,"form_action"=>$form_action));
			//set layout
			$this->setView("Views/Backend/layout.php");
			//xuat MVC len man hinh
			$this->renderBody();
		}
		//do edit user
		public function doEdit(){
			$this->executeEdit(); //ham nay trong class productModel
			//quay tro lai trang user
			header("location:admin/list-product");
		}
		//add user
		public function add(){
			$form_action="index.php?area=admin&controller=product&action=doAdd";
			//set duong dan cua file tempate
			$this->setLayout("Views/Backend/layout.php");
			//day noi dung MVC vao bien view (muon truyen bien nao ra view thi dua vao danh sach array)
			/*
			...
			*/
			$this->setView("Views/Backend/add_edit_product.php",array("form_action"=>$form_action));
			//hien thi noi dung len man hinh
			$this->renderBody();
		}
		//do edit user
