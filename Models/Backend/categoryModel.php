	...
	*/
	trait categoryModel{
		...
		public function executeEdit(){
			...
			$query = $db->prepare("update tbl_category set name=:name, slug=:slug where category_id=:id");
			$query->execute(array("name"=>$name,"slug"=>$slug,"id"=>$id));
		}
		public function executeAdd(){
			...
			$query = $db->prepare("insert into tbl_category set name=:name, slug=:slug");
			//truyen cac bien tuong ung voi cau truy van. VD :password trong cau sql thi phai truyen bien password vao
			$query->execute(array("name"=>$name,"slug"=>$slug));
			//-------------------------		
		}
