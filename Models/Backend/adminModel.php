<?php 
	/*
	* File name: Models/Backend/adminModel.php
	* Loai file: file model trong mo hinh MVC	
	*/
	trait adminModel{
		//ham lay danh sach cac ban ghi
		public function fetchAll($from, $recordPerPage){//ham fetchAll cua class adminModel
			//lay doi tuong connection de thao tac csdl
			$db = connection::getInstance();			
			//chuan bi truy van
			$query = $db->prepare("select * from tbl_admin order by id desc limit $from, $recordPerPage");
		...
		public function total(){
			...
			$query_total = $db->prepare("select id from tbl_admin");
			$query_total->execute();
		...
		public function fetchOne(){
			...
			$query = $db->prepare("select * from tbl_admin where id=:id");
			//lay theo kieu object
			$query->setFetchMode(PDO::FETCH_OBJ);
			//thuc thi truy van
		...
		public function executeEdit(){
			...
			$query = $db->prepare("update tbl_admin set fullname=:fullname, email=:email where id=:id");
			...
			if($password != ""){
				...
				$query = $db->prepare("update tbl_admin set password=:password where id=:id");
		...
		public function executeAdd($arr){
			...
			$db = connection::getInstance();
			//update email, fullname
			$query = $db->prepare("insert into tbl_admin set fullname=:fullname , email=:email, password=:password");
		...
		public function executeDelete(){
			...
			$db = connection::getInstance(); //Core/connection.php
			//update email, fullname tuong ung voi ban ghi co id truyen vao
			$query = $db->prepare("delete from tbl_admin where id=:id");
