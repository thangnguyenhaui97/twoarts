	...
	*/
	trait productModel{
		...
		public function executeEdit(){
			...
			$query = $db->prepare("update tbl_product set name=:name, slug=:slug, category_id=:category_id, type_id=:type_id, quantity=:quantity, price=:price, discount=:discount, description=:description, content=:content, hotproduct=:hotproduct where id=:id");
			$query->execute(array("name"=>$name,"slug"=>$slug,"category_id"=>$category_id, "type_id"=>$type_id, "quantity"=>$quantity,"price"=>$price,"discount"=>$discount, "description"=>$description, "content"=>$content, "hotproduct"=>$hotproduct,"id"=>$id));
		...
		public function executeAdd(){
			//cac bien la form-control trong view		
			$name = $_POST["name"];
			$slug = db::remove_unicode($_POST["name"]);
			$category_id = $_POST["category_id"];
			$price = $_POST["price"];
			$type_id = $_POST["type_id"];
			$quantity = $_POST["quantity"];
			...
			$db = connection::getInstance(); //Core/connection.php
			//update email, fullname tuong ung voi ban ghi co id truyen vao
			$query = $db->prepare("insert into tbl_product set name=:name, slug=:slug, category_id=:category_id, type_id=:type_id, quantity=:quantity, price=:price, discount=:discount, description=:description, content=:content, hotproduct=:hotproduct, img=:img");
			$query->execute(array("name"=>$name,"slug"=>$slug,"category_id"=>$category_id, "type_id"=>$type_id, "quantity"=>$quantity,"price"=>$price,"discount"=>$discount,"description"=>$description, "content"=>$content,"hotproduct"=>$hotproduct,"img"=>$img));
			//-------------------------		
		}
